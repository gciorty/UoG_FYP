...
const web3 = require("../providers/web3.ethash");
...
beforeEach(async () => {
  ...
  await web3.eth.personal.unlockAccount(accounts[0], "Gabriel1234!", 15000);
});

describe("POW Testing Network -> Deployment", async () => {
  ...
  it("Adds 10 votes randomly (Set OP)", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      if (Math.random() >= 0.5) {
        // return true or false to randomly select first or second option - no footprint
        await election.methods
          .castVote(option1)
